...
class M_PSG2FEAT(torch.nn.Module):
    def __init__(self, config):
        ...
        self.return_att_weights = config.return_att_weights
...
#        self.transformer = nn.TransformerEncoderLayer(256, nhead = 8)
    ...
    def summary(self, input_size, device, batch_size=-1):
        ...
        if device.type == 'cuda':
            ...
            total_input_size = abs(np.sum([np.prod(x) for x in input_size]) * batch_size * 4.0 / 1048576.0)
